#Hu is the most parsimonious common true hypothesis (MPCTH) because studies come from different populations (H1 or Hc)
  ...
  for(s in 1:length(n)){
    ...
    for(r in 1:length(ratio_HiHc)){
      ...
      for(t in 1:studies){
        ...
        print(paste("MPCTH(Hu):: Iteration i:",i, "Sample size s:", s, "Study t:", t, "Ratio r:",r))
...
# Aggregate PMPs--------------------------------------------------------------------
load("RRrepo/workspaces/Simulate BFs_ES(H1)=ES(Hc).RData")
...
PMP_HuTRUE_eqES<-list()
for(i in 1:length(all.hyps)){
  PMP_HuTRUE_eqES[[all.hyps[i]]] <- aggregatePMP(BF=BF, #the simulated BFs from the simulate_BF_.R scripts
...
PMP_HuTRUE_eqES[c("iter", "n_studies", "n", "ratio_beta.Hi","ratio_beta.Hc", "r2", "pcor", "models")]<-list(iter, studies, n, ratio_beta.Hi, ratio_beta.Hc, r2, pcor, models)
