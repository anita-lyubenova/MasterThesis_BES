...
res1par<-readRDS(file= "simulation/output/res1par.rds")
length(res1par)
...
#Output: the same list with 135 elements, but the elements are named
...
#Output:
# a nested list with:
#     15 elements at level 1 (conditions) [beta_mu x beta_tau];
#     9  elements at level 2 [sample sizes n], where each element is a dataframe [60000, 2] 
...
beta_mu<- gsub(".*beta_mu(.+)_beta_tau.*", "\\1", pop_names) %>% unique()
beta_tau<-gsub(pattern = ".*_beta_tau", "", pop_names) %>% unique()
...
saveRDS(BF_bind3, file="pre-processing/output/BF_data_1par.rds")
