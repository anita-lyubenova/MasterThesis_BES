...
median.PMP1.n100.noHu.plot<-median.PMP1.n100.noHu.df%>% 
  ...
  labs(#title = "Aggregate PMPs for each hypothesis with increasing number of studies",
    ...
    y="Aggregate PMP")+
  theme(text = element_text(size = 9),
        axis.text.x = element_text(size = 7, colour = rep(c(col.H1,col.Hc), times=20)),
        legend.title = element_text(size = 7)
        # plot.subtitle = element_text(size = 12),
        # legend.text = element_text(size = 10),
  )+
...
library(ggpubr)
library(patchwork)
F1<-ggarrange(median.PMP1.noHu.plot+ rremove("xlab"),
             median.PMP1.plot,
             labels = c("a)", "b)"),
             legend="bottom",
             nrow=2
...
F2<-median.PMP2.noHu.plot/median.PMP2.plot+
  ...
  theme(legend.position='bottom')
F2
ggsave("Outputs/Research Report/F2.png", plot = F2, width = 7, height = 5, units = "in", dpi = 300, bg="white")
...
ggsave("Outputs/Research Report/Fig1_v3.png", plot = b, width = 9, height = 4.6, units = "in", dpi = 300, bg="white")
...
#Figure 2 -------------------------------
support.df<-cbind(colSums(support.Hu[,,2])/iter,colSums(support.Hu2[,,2])/iter)
colnames(support.df)<-c("1:1", "3:1")

support.plot<-support.df %>% 
as.data.frame() %>% 
  rownames_to_column(var = "t") %>% 
...
ggsave("Outputs/Research Report/support.plot.png", plot = support.plot, width = 7, height = 2, units = "in", dpi = 300, bg="white")



support.plot1<-support.df %>% 
  as.data.frame() %>% 
  rownames_to_column(var = "t") %>% 
  pivot_longer(cols = colnames(support.df),
               names_to = "ratio",
  ...
  filter(ratio=="1:1") %>% 
  ggplot(aes(x=t, y=prop.highest.PMP_u, group=1))+
  geom_point()+
  geom_line()+
  theme_minimal()+
  labs(x="Size of the study set ",
...
ggsave("Outputs/Research Report/Presentation/Fig3.png", plot = support.plot1, width = 5, height = 2, units = "in", dpi = 300, bg="white")
